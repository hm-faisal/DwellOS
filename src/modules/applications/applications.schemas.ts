...
export const updateApplicationStatusSchema = z.object({
	...
	body: z.object({
		status: z.enum(['UNDER_REVIEW', 'INFO_REQUESTED', 'REJECTED', 'WITHDRAWN']),
		notes: z.string().trim().max(2000).optional(),
	}).strict(),
});
...
export const approveApplicationSchema = z.object({
	...
	body: z.object({
		...
		billingCycle: z.enum(['MONTHLY', 'WEEKLY', 'BIWEEKLY']).default('MONTHLY'),
...
export type UploadDocumentInput = z.infer<typeof uploadApplicationDocumentSchema>['body'];
export type UpdateStatusInput = z.infer<typeof updateApplicationStatusSchema>['body'];
