...
export const createRoomSchema = z.object({
	...
	body: z.object({
		...
		rent: z.number().int().nonnegative('Rent must be in integer minor units (cents)').max(100000000),
		deposit: z.number().int().nonnegative('Deposit must be in integer minor units (cents)').max(100000000),
...
export const listPropertyRoomsSchema = z.object({
	...
	query: z.object({
		...
		status: z.enum(['AVAILABLE', 'RESERVED', 'OCCUPIED', 'MAINTENANCE', 'ARCHIVED']).optional(),
	}).strict().optional(),
});
